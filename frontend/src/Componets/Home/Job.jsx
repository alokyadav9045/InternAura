...
function Job() {
    ...
    useEffect(()=>{
        const fetchData= async()=>{
            try {
            const response= await axios.get(`https://intern-aura-2dwiql35l-alokyadav9045s-projects.vercel.app/api/job`)
  ...
  return (
    ...
    <div className="mt-16">
            <h1 className='text-center font-bold'>Latest Jobs on InternAura</h1>
        </div>
...
filterInternShips.map(( data,index)=>(
        ...
        <div className="int-1 mt-2  bg-slate-300" key={index}>
<p className='mb-4 mt-7' id='boxer'> <i className='bi bi-arrow-up-right text-blue-500' ></i> Actively Hiring</p>
<p>{data.title}</p>
